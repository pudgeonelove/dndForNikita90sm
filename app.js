...
function renderEncounter(node) {
  ...
  if (node.type === "event") {
    actions.innerHTML = `
      <button class="button button--primary" type="button" data-action="event-roll">Бросить d20</button>
      <button class="button" type="button" data-action="complete-node">Пройти осторожно</button>
    `;
  } else if (node.type === "camp") {
    actions.innerHTML = `
      <button class="button button--primary" type="button" data-action="camp-heal">Лечиться</button>
      <button class="button" type="button" data-action="complete-node">Уйти дальше</button>
    `;
  } else if (node.type === "shop") {
    actions.innerHTML = `
      <button class="button button--primary" type="button" data-action="buy-potion">Купить зелье -18 gp</button>
